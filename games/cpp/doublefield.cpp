 ...
 */
DoubleMeanFieldGame::DoubleMeanFieldGame(size_t size, double _b1, double _b2): AbstractSpatialGame(size, (_b1+_b2)/2){
    L = size;
 ...
 */
void DoubleMeanFieldGame::set_field(const std::vector<int> &new_field1, const std::vector<int> &new_field2){
...
//    field.assign(new_field.begin(), new_field.end());
    densities.clear();
    unchanged.assign(2*L*L, 1);
    densities.push_back(static_cast<double>(accumulate(new_field1.begin(),new_field1.end(),0))/new_field1.size());
    densities.push_back(static_cast<double>(accumulate(new_field2.begin(),new_field2.end(),0))/new_field2.size());
}
...
void DoubleMeanFieldGame::update_field(const std::vector<double> &scores, int time_moment, int perCalFrom, int perCalTill){
    //Field
        ...
        for (size_t k = 0; k < L*L; k++) {
            ...
            for (int i = -1; i <= 1; i++) //Row
            {
                for (int j = -1; j <= 1; j++) //Col
                {
                    ...
                    if (scores[offset + bestStrategyIndex] < scores[offset + memberIndex])
                    {
 ...
 * Evolve num_steps
 */
void DoubleMeanFieldGame::evolve(int num_steps, int perCalFrom, int perCalTill)
{
    std::vector<double> scores(2*L*L, 0);

    for(int step = 0,time_moment = densities.size()/2; step < num_steps; step++, time_moment++)
    {
