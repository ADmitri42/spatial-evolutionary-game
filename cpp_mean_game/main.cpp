...
int main() {
    ...
    for(int i = 0; i < bs.size(); i++){
        ...
        for(int j = 0; j < 40; j++){
            ...
            densities.insert(densities.end(), new_den.begin()+10001, new_den.end());
            new_den.clear();
        }
        ...
        cout << "Saving data: ";
        cnpy::npy_save("data/density_of_cooperators5.npy",&densities[0],{bs.size(), i+1,8000},"w");
        cout << "done" << endl;
    }
    cnpy::npy_save("data/density_of_cooperators5.npy",&densities[0],{bs.size(),40,8000},"w");
    return 0;
}