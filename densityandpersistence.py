...
parser.add_argument('-y', dest='overwrite',
                    action='store_true', help="If needed, answer yes")
...
if config["GameType"] == "DoubleMeanFieldGamePy":
    ...
    if type(config["parameters"]) == dict:
        bs = list(product(config["parameters"]["b1"],
                          config["parameters"]["b2"]))
    else:
        bs = list(product(config["parameters"], config["parameters"]))
    persistence_shape = (len(bs), config["fields"]["quantity"], 2)
    density_shape = (
        len(bs), config["fields"]["quantity"], 2, config["persistence"]["end"] + 1)
    game = GamePy(config["fields"]["size"], 1.3, 1.3,
                  config["persistence"]["start"], config["persistence"]["end"])
else:
    bs = config["parameters"]
    persistence_shape = (len(bs), config["fields"]["quantity"])
    density_shape = (
        len(bs), config["fields"]["quantity"], config["persistence"]["end"] + 1)
    game = GamePy(config["fields"]["size"], 1.3,
                  config["persistence"]["start"], config["persistence"]["end"])
...
if config["GameType"] == "DoubleMeanFieldGamePy":
    if type(config["parameters"]) == dict:
        ...
        indeces = product(range(len(config["parameters"]["b1"])), range(
            len(config["parameters"]["b2"])))
    else:
        new_density = np.zeros((len(config["parameters"]), len(
            config["parameters"]), config["fields"]["quantity"], config["persistence"]["end"] + 1, 2))
        new_persistence = np.zeros((len(config["parameters"]), len(
            config["parameters"]), config["fields"]["quantity"], 2))
        indeces = product(range(len(config["parameters"])), range(
